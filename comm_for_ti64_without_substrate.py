...
#
# MODIFICATIONS:
# - Removed substrate geometry and associated boundary groups (bottoms, tops, sides).
# - Boundary conditions (fixed displacement & temp) are now applied to the
#   'bottom' group of the build geometry.
# - Implemented efficient file I/O using DEFI_FICHIER with 'LIBERER' action.
# - Switched result file output to 'BINARY' format for better performance.
#
# ==============================================================================
...
output_filename = "lpbf_n_layer_cooling_no_substrate.comm"
...
baseplate_temp = 373.15   # Constant temperature of the bottom of the build (K)

# --- Output Units ---
# We now use two fixed units and liberate them after each use for efficiency.
ther_output_unit = 61
meca_output_unit = 21
...
def generate_comm_file():
    ...
    for i in range(1, num_layers + 1):
        ...
        model_ther, model_meca = f"model{i}", f"modmeca{i}"
        mat_ther_assign, mat_meca_assign = f"assth{i}", f"assmec{i}"
        list_inst, list_res = f"listr{i}", f"listres{i}"
        temp_load, fixmec_load = f"chtmp{i}", f"fixmec{i}"
        temp_init_new_layer = f"tmp{i}"
        res_ther, res_meca = f"resther{i}", f"resmec{i}"
        stress = f"stress{i}"
        name_ther_res, name_mec_res = f"ther{i}", f"mec{i}"
        prev_res_ther, prev_res_meca = f"resther{i-1}", f"resmec{i-1}"
    
        # --- Define active groups for the current step ---
        # 1. Physical groups (with material): all deposited layers.
        physical_groups_list = [f"'layer{j}'" for j in range(1, i + 1)]
        physical_groups_str = f"({', '.join(physical_groups_list)}, )"
    
        # 2. Model groups: physical groups + boundary condition groups.
        #    The only BC group is 'bottom' for fixing the base of the build.
        model_groups_list = physical_groups_list + ["'bottom'"]
        ...
        # Apply constant temperature and displacement to the 'bottom' of the build.
        add_line(f"{temp_load} = AFFE_CHAR_THER(MODELE={model_ther}, TEMP_IMPO=_F(GROUP_MA=('bottom', ), TEMP={baseplate_temp}))")
        ...
        if i == 1:
            add_line("# For layer 1, the initial state is just the hot new layer.")
            etat_init_ther_str = f"ETAT_INIT=_F(CHAM_NO={temp_init_new_layer}),"
            etat_init_meca_str = ""
        else:
            add_line(f"# --- State Transfer from Layer {i-1} to Layer {i} ---")
            # Groups from the previous step (all layers except the current one)
            ...
            field_stress_new, field_stress_ext, field_stress_init = f"strfield{i}_1", f"strfield{i}_2", f"strfield{i}_3"
        ...
        add_line(f"# --- Save and release thermal results for layer {i} ---")
        add_line(f"DEFI_FICHIER(ACTION='ASSOCIER', FICHIER=r'{simulation_path}/{name_ther_res}.rmed', UNITE={ther_output_unit}, TYPE='BINARY')")
        add_line(f"IMPR_RESU(UNITE={ther_output_unit}, RESU=_F(LIST_INST={list_res}, {impr_ther_options}))")
        add_line(f"DEFI_FICHIER(ACTION='LIBERER', UNITE={ther_output_unit})\n")
    
        add_line(f"# --- Save and release mechanical results for layer {i} ---")
        add_line(f"DEFI_FICHIER(ACTION='ASSOCIER', FICHIER=r'{simulation_path}/{name_mec_res}.rmed', UNITE={meca_output_unit}, TYPE='BINARY')")
        add_line(f"IMPR_RESU(UNITE={meca_output_unit}, RESU=(_F(LIST_INST={list_res}, NOM_CHAM=('DEPL',), RESULTAT={res_meca}), _F(LIST_INST={list_res}, {impr_meca_options})))")
        add_line(f"DEFI_FICHIER(ACTION='LIBERER', UNITE={meca_output_unit})")
